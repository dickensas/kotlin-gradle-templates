...
var FONT_WITH_MANUAL_SIZE = "Sans Bold 18"
...
fun main(args: Array<String>) = memScoped {
    ...
    var temp_surface = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, 0, 0) 
    var layout_context = cairo_create (temp_surface)
    cairo_surface_destroy (temp_surface)
    
    //#2 create pango layout
    var layout = pango_cairo_create_layout(layout_context)
    
    //#3 set the text
    pango_layout_set_text(layout, TEXT, -1)
    
    //#4 load the font
    var desc = pango_font_description_from_string(FONT_WITH_MANUAL_SIZE);
    pango_layout_set_font_description(layout, desc)
    pango_font_description_free(desc)
    
    //#5 get text dimension
    var text_width = alloc<IntVar>()
    var text_height = alloc<IntVar>()
    pango_layout_get_size (layout, text_width.ptr, text_height.ptr)
    var awidth = (text_width.value/PANGO_SCALE)
    var aheight = (text_height.value/PANGO_SCALE)
    ...
    //#7 paint the text inside the cairo render context
    cairo_set_source_rgba (render_context, 1.toDouble(), 1.toDouble(), 1.toDouble(), 1.toDouble())
